...
{/literal}
<h2>Reporte Kardex Fisico {if $USER_ROL eq 1}Valorado{/if}: Del <b>{$inicio|date_format:"%d-%m-%Y"}</b> Al: <b>{$fin|date_format:"%d-%m-%Y"}</b>
{if $USER_ROL eq 1}(En {if $moneda eq 0}Bolivianos.{else}Dolares Americanos.{/if}){/if}</h2>
<form action="{$module}" method="post">
<input type="hidden" value="{$id}" name="id" />

<table  class="search" align='center'  border="1" cellspacing="0" cellpadding="5">
<tr>
...
<ul class="bar">
  <li><a  href="{$module}&type=2&cat={$cateId}&codigo={$codigo}&inicio={$inicio}&fin={$fin}&numLineas={$numeroLineas}&moneda={$moneda}" target="_blank" title="Imprimir" >
    <img src="template/images/icons/printer.png"  border="0"/>Imprimir Kardex Fisico {if $USER_ROL eq 1}Valorado{/if}: Del <b>{$inicio|date_format:"%d-%m-%Y"}</b> Al: <b>{$fin|date_format:"%d-%m-%Y"}</b>
{if $USER_ROL eq 1}(En {if $moneda eq 0}Bolivianos.{else}Dolares Americanos.{/if}){/if}</a></li>
  </ul>
<table  class="formulario" align='center'  border="0">
  ...
  <tr class="{$fila}" onMouseOver="this.className='lista3'; return true;" onMouseOut="this.className='{$fila}'; return true;">
    ...
    <td align="left">
      {if $item[i].tipoComprobante eq "V"}  
     <a href="index.php?module=seller&action=recibo&id={$item[i].itemId}" target="_blank" title="Ver comprobante N&deg; {$item[i].comprobante}"> {$item[i].tipoComprobante}&nbsp;{$item[i].comprobante}</a>
    {elseif  $item[i].tipoComprobante eq "T"}
     <a href="index.php?module=reception&action=viewRecep&id={$item[i].itemId}"  target="_blank" title="Ver comprobante N&deg; {$item[i].comprobante}"> {$item[i].tipoComprobante}&nbsp;{$item[i].comprobante}</a>
    {elseif  $item[i].tipoComprobante eq "C" }<a href="index.php?module=salida&action=recibo&id={$item[i].itemId}"  target="_blank" title="Ver comprobante N&deg; {$item[i].comprobante}">     {$item[i].tipoComprobante}&nbsp;{$item[i].comprobante}</a>
    {elseif  $item[i].tipoComprobante eq "SI"}
    &nbsp;
     {elseif  $item[i].tipoComprobante eq "A"}
    {$item[i].tipoComprobante}&nbsp;{$item[i].comprobante}
    {/if}
  
    </td>
    <td align="left" nowrap="nowrap">{$item[i].dateReception}</td> 
    <td align="left">{$item[i].descripcion}
    </td>
    <td align="right">{$item[i].tipoCambio}</td>
    <td align="right" bgcolor="#EEFDB0">{if $item[i].tipo eq "I"}{$item[i].amount|number_format:2:'.':','}{/if} </td>
    ...
    <td align="right">{if $item[i].tipo eq "I"} {$item[i].montoTotal|number_format:2:'.':','}{/if}</td>
...
{*dolar*}
    ...
    <td align="right" class="dolar">{if $item[i].tipo eq "I"} {$item[i].costoTotalDolar}{/if}</td>
    <td align="right" class="dolar">{if $item[i].tipo eq "S"} {$item[i].costoTotalDolar}{/if}</td>
    <td align="right" class="dolar"> {if $item[i].saldoDolar < 0 }<span style="color:red"> {$item[i].saldoDolar}</span>{else}{$item[i].saldoDolar}{/if}</td>
    {/if}
    {/if}
  </tr>
  ...
  {if $item[i].productId neq $item[i.index_next].productId}    
         <tr  style="border-bottom:1px #000 solid;"  >
           ...
           {elseif $moneda eq 1}           
           <td align="right">&nbsp;</td>         
           <td align="right">{$ingresoDolar}</td>
           <td align="right">{$salidaDolar}</td>
           <td align="right">{$saldoDolar}</td>
           {/if}
           {/if}
         </tr>
