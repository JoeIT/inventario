...
<table width="100%" border="0" cellpadding="2" cellspacing="0" class="formulario">
  <tr>
    <th colspan="4">Comprobante de Ingreso - {if $recibo.tipoComprobante == "C"}Compra Local 
  ...
  <tr>
    ...
    <td>{if $recibo.tipoComprobante == "C"}Compra Local 
        ...
        {else}Compra Importada
        {/if}</td>
    <td class="titulo">{if $recibo.tipoComprobante == "T"}    Origen
  ...
  <tr>
    ...
    <td>{$impuesto.name}</td>
    <td class="titulo">{if $recibo.tipoComprobante == "T"}Documento
