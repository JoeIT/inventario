...
<table   border="1" align='center' cellspacing="0" class="formulario" width="100%" >
  ...
  <tr>
    ...
    {$item.productId}
      <input type="hidden" name="id" id="textfield"  value="{$item.ingresoId}"/>
        ...
        <input type="hidden" name="comprobanteId" id="textfield"  value="{$item.itemId}"/>
   </td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td><input type="text" name="item[priceReal]" id="price"  class="numero" value="{$item.priceReal}"   onchange="calcular(this.value,cantidad.value)"/></td>
  </tr>
  <tr>
    <td scope="row">Importe</td>
    <td><input type="text" name="item[totalReal]" id="total"  class="numero"  value="{$item.totalReal}" readonly="readonly"/></td>
  </tr>
  <tr>
