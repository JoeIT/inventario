...
<input type="hidden" name="item[encargado]" id="encargado" value="{$userName}">
...
<table border="0" class="formulario"  width="100%" align="center">
  ...
  <tr>
    ...
    <td width="44%" nowrap="nowrap">  <input name="item[dateReception]" type="text"  class="fecha" id="reception" value="{$smarty.now|date_format:'%Y-%m-%d'}" readonly="readonly">
    </label>
  ...
  <tr>
    ...
    <td>
      <select name="item[tipoComprobante]" id="tipoComprobante" style="width:150px" onchange="tipoTransferencia(this.value)">
        <option value="P">{#traspasoHaciaProduccion#}</option>
        <option value="TS">{#traspasoHaciaSucursal#}</option>
    ...
    <select name="salida[destinoId]" id="salida" style="width:150px" onchange="setReferencia();">        
      {section name=i loop=$almacenes}             
		  <option value="{$almacenes[i].almacenId}">{$almacenes[i].code} {$almacenes[i].name}</option>        
      {/section}               
	</select>
    {/if}
  ...
  <tr>
    ...
    <td colspan="3"><input name="item[tipoCambio]" type="text" id="tipoCambio" value="{$tipoCambio}" readonly="readonly"  class="numero"/>Bs. A la fecha:<div id="tipoCambioLabel" style="display:inline">[{$lastUpdate}]</div><div id="test"></div></td>
  </tr>
  <tr>
