...
class puntoVenta extends Principal
{
    ...
    function getComprobante($id)
    {
        ...
        $sql = " select v.*,c.* , IF (v.tipoPago=0,'Efectivo',IF(v.tipoPago=1,'Tarjeta credito/debito',IF(v.tipoPago=2,'Credito',IF(v.tipoPago=3,'Cheque','Deposito Banco')))) as tipoPagoVenta
    ...
    function getListSalidas($tipo="",$inicio,$fin)
    {
        ...
        {
            $sql = " select v.*,c.*,IF (v.tipoPago=0,'Efectivo',IF(v.tipoPago=1,'Tarj. credito/debito',IF(v.tipoPago=2,'Credito',IF(v.tipoPago=3,'Cheque','Deposito Banco')))) as tipoPagoVenta ";
  ...
  /*  function addItems($rec)
      ...
      function ordenarComprobantesVentas($inicio="",$fin="")
    {
       ...
       /* $sql = " select r.itemId,r.comprobante, r.dateReception, r.tipoComprobante ";
        ...
        /**
         * result = 0
         * = 1 Existen comprobantes bloqueados
         * =
        **/
        $result = 0;
        
        //verificar que no existan comprobantes bloqueados o cerrados
        
       $sql = "select count(r.state) as bloqueados
                from almacen_reception r where r.almacenId = ".$_SESSION["almacenId"]."
                and r.tipoComprobante = 'V' and r.dateReception >='$inicio' 
                and r.state =1        
                ";
       	$infoBloqueados = $db->execute($sql);
                    
        if ($infoBloqueados->fields("bloqueados")==0)
        {
            
            //obtener ultimo numero comprobante antes de la fecha dada
            $sql = "select r.itemId,r.comprobante, r.dateReception, r.tipoComprobante,v.numeroFactura 
                    from almacen_reception r, almacen_ventas v where r.almacenId =".$_SESSION["almacenId"]."
                    and v.itemId = r.itemId and r.tipoComprobante = 'V' and r.dateReception <'$inicio' 
                    order by r.tipoComprobante, r.dateReception desc, v.numeroFactura desc
                    limit 0,1";
            
            $infoLastComprobante = $db->execute($sql);
            $nroComprobante =  $infoLastComprobante->fields("comprobante");
            
            
            
            //listar todos los comprobantes de venta a ser ordenados desde la fecha inicial
            $sql = " select r.itemId,r.comprobante, r.dateReception, r.tipoComprobante ";
            $sql.= " from almacen_reception r, almacen_ventas v ";
            $sql.= " where r.almacenId = ".$_SESSION["almacenId"];
            $sql.= " and v.itemId = r.itemId ";
            $sql.= " and r.tipoComprobante = 'V'";  
            $sql.= " and r.dateReception >='$inicio'";      
            $sql.= " order by r.tipoComprobante, r.dateReception asc, v.numeroFactura asc ";  
            ...
            for ($i=0; $i<$num; $i++)
            {
               ...
               // echo "$i ".$item[$i]["dateReception"]." - ".$item[$i]["comprobante"]." Contador -> ".$contador."<br>"; 
            }
            
            //registrar log del ordenamiento de comprobantes
            
            $log["dateReport"] = date("Y-m-d H:i:s");
            $log["dateStar"] = $inicio;
            $log["dateEnd"] = date("Y-m-d");
            $log["total"] = $num;
            $log["user"] = $_SESSION["userId"];
            $log["tipo"] = "Ventas";
            $db->AutoExecute("report_ordenamiento",$log);
           
        }elseif($infoBloqueados->fields("bloqueados")>0)
        {
            $result = 1;
        }            
        return $result;            
