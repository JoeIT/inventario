...
class Moneda 
{
   ...
   /* function saveTipoCambio($rec,$id)
      ...
      function updatePriceDolar($tipoCambio)
    {
		...
		$ultimoTipoCambio = $this->getUltimoTipoCambio($fechaActual);
		
		
	
		if ($tipoCambio == $ultimoTipoCambio)
			{
				//mantener tipo cambio
				//registrar los nuevos datos para los nuevos productos
				$sql = " SELECT p.*
						 FROM product_item as p LEFT JOIN log_precioventas as l ON p.productId = l.productId
						 where l.productId is NULL ";
				$sql.= " and p.almacenId = ".$_SESSION["almacenId"];
				...
				for ($i=0; $i<count($item); $i++)
				{
					...
					$db->AutoExecute("product_item",$rec,'UPDATE',"productId = '".$item[$i]["productId"]."'");
						...
						$logPrecio["tipoCambio"] = $tipoCambio;
	...
	/**
	Obtener ultimo tipo de cambio anterior a una fecha dada
	*/
	 function getUltimoTipoCambio($fecha)
    {
        global $db;
        $sql = " select *, date_format(dateRefresh,'%d-%m-%Y')  as fechaTipoCambio from moneda_tc";
        $sql.= " where dateRefresh <'$fecha' ";
