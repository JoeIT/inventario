...
class Produccion
{
    ...
    function Produccion()
    {
        $this->table = "produccion_item";
        $this->tableId = "produccionId";
    ...
    function getNextOrdenProduccion()
    {
        global $db;
        $sql = " select count(*) as total from ".$this->table;
        ...
        $db->SetFetchMode(ADODB_FETCH_ASSOC);
		$info = $db->execute($sql);
        $nextOrden = $info->fields["total"]+1;
        return $nextOrden;
    }
    function closeOrden($id)
    {
        $rec["active"] = 0;
        $rec["dateUpdate"] = date("Y-m-d H:s:i");
        $rec["dateClose"] = date("Y-m-d H:s:i");
        $this->updateItem($rec,$id);
    }
