 ...
 * */
 ...
 else if ($action == "recibo")
 {
    ...
    if ($_GET["type"] == 2)//opcion de imprimir
    {
        ...
        $smarty->assign("titulo","NOTA DE ENTREGA");         
  ...
  else if ($action == "searchItem")
 {
    ...
    if(isset($_REQUEST['queryString'])) {
        ...
        for ($i=0; $i<count($items); $i++)
        {
            ...
            $descripcion = $items[$i]["categoria"]." ".$nombre." ".$items[$i]["color"];
            $descripcion2 = $items[$i]["categoria"]." ".$nombre2." ".$items[$i]["color"];
            $funcion = "'".$items[$i]["productId"]."','".$descripcion."','".$items[$i]["stock"]."','".$precioPonderado."','".$items[$i]["unidad"]."','".$items[$i]["precio"]."','".$items[$i]["precioDolar"]."','".$items[$i]["codebar"]."'";
 ...
 elseif ($action == "order")
 {
    ...
    $smarty->assign("content",$templateDirModule."/formOrdenar.tpl");
    $template = "modal.tpl"; 
 ...
 elseif($action == "orderUpdate")
 {
    ...
    $result = $objItem->ordenarComprobantesVentas($dateInit);
    $smarty->assign("dateInit",$dateInit);
