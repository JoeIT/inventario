 ...
 * */
 ...
 else if ($action == "add")
 {
    ...
    $id = $objItem->saveComprobante($item,$venta); // registro de los datos del comprobante
    
    if ($_POST["codigo"]!="" && $_POST["cantidad"]!=0 && $_POST["precio"]!=0)
    {
        $rec["itemId"] = $id; //registro del item vendido        
  ...
  else if ($action == "searchItem")
 {
    ...
    if(isset($_REQUEST['queryString'])) {
        ...
        for ($i=0; $i<count($items); $i++)
        {
            ...
            //$descripcion.= " Dolar --".$items[$i]["precioDolar"];
         /*   $descripcion.= " ".$_REQUEST["fechaComp"];
            $descripcion.= " TC: ".$_REQUEST["tipoCambio"];
            $descripcion.= " costo".$precioPonderado;*/
            $funcion = "fill('".$items[$i]["productId"]."','".$descripcion."','".$items[$i]["stock"]."','".$precioPonderado."','".$items[$i]["unidad"]."','".$items[$i]["precio"]."','".$items[$i]["precioDolar"]."')";            
